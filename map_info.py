...
def levenshtein(s, t, ratio_calc=False):
    ...
    """
    global map_info_row, map_info_col
    ...
    for map_info_col in range(1, cols):
        for map_info_row in range(1, rows):
            if s[map_info_row - 1] == t[map_info_col - 1]:
                cost = 0
            else:
                if ratio_calc:
                    cost = 2
                    ...
                    cost = 1
            distance[map_info_row][map_info_col] = min(distance[map_info_row - 1][map_info_col] + 1,
                                                       distance[map_info_row][map_info_col - 1] + 1,
                                                       distance[map_info_row - 1][map_info_col - 1] + cost)
    if ratio_calc:
        ratio = ((len(s) + len(t)) - distance[map_info_row][map_info_col]) / (len(s) + len(t))
        return ratio
    else:
        return "The strings are {} edits away".format(distance[map_info_row][map_info_col])
...
def main_def():
    ...
    grid_step_x, grid_step_y = float(grid_step[0]), float(grid_step[1])
    ...
    tr_lat = 0
    tr_lon = map_total_x / scalar_lon_max * 1
...
def get_info(show=False):
    ...
    try:
        ...
        for i in maps.keys():
            ...
            _dict[i] = levenshtein(_hash, hex(i), ratio_calc=True)
        ...
        for i in _dict.keys():
            ...
            if _dict[i] == max(_dict.values()):
                map_info_match = maps[i]
                percent_match = _dict[i]
    finally:
        map_info_match = map_info_match[:-4].replace('_', ' ').title()
        for map_type in ['Avg', 'Avn', 'Air']:
            if map_type in map_info_match:
                map_info_match = map_info_match.replace(map_type, f"[{map_type.upper()}]")
        if show:
            loguru.logger.debug(f"[L] LOOKUP HASH  : {_hash.upper()}")
            if percent_match:
                percent_match *= 100
                loguru.logger.debug(f"[L] LOOKUP MATCH : {str(percent_match)[:5]}%")
            else:
                loguru.logger.debug(f"[L] LOOKUP MATCH : 100.00%")
            loguru.logger.debug(f"[L] REGION IDENT : {map_info_match}")
        if os.path.exists('map.jpg'):
            os.remove('map.jpg')
